...
export const HistoryDataPanel: React.FC<HistoryDataPanelProps> = ({ deviceId, realtimeData }) => {
  ...
  const processChartData = (data: HistoryDataEntry[]) => {
    ...
    // 按时间分组，每秒只保留第一条数据
    const timeGroupedData = new Map<string, HistoryDataEntry>()
    ...
    sortedData.forEach(entry => {
      ...
      // 如果这个时间点还没有数据，则添加（即每秒只保留第一条）
      if (!timeGroupedData.has(time)) {
        timeGroupedData.set(time, entry)
      }
    })

    // 处理去重后的数据
    timeGroupedData.forEach(entry => {
      const time = new Date(entry.received_at).toLocaleTimeString()
  ...
  useEffect(() => {
    ...
    if (timeSinceLastUpdate < 1000) {
      // 如果距离上次更新不足1秒，将数据存储到pending中，不立即更新
      console.log('数据更新间隔不足1秒，存储到pending:', timeSinceLastUpdate)
      pendingDataRef.current = realtimeData
      return
    }
    ...
    // 处理数据更新
    processRealtimeDataUpdate(realtimeData)
  }, [realtimeData])
  ...
  useEffect(() => {
    updateIntervalRef.current = setInterval(() => {
      ...
      // 如果有待处理的数据且距离上次更新已经超过1秒
      if (pendingDataRef.current && timeSinceLastUpdate >= 1000) {
        console.log('定时处理待更新数据')
        lastUpdateTimeRef.current = now
        processRealtimeDataUpdate(pendingDataRef.current)
  ...
  // 处理实时数据更新的具体逻辑
  const processRealtimeDataUpdate = (realtimeData: any) => {
    // 创建实时数据点
    const now = new Date()
  ...
  const createChartConfig = (data: ChartDataPoint[], fixedYAxis?: { min: number; max: number }) => {
    ...
    return {
      ...
      animation: {
        ...
        leave: false, // 禁用离开动画，避免线条消失时的怪异效果
      },
      // 禁用一些可能导致动画异常的配置
      interactions: [
        {
      ...
      tooltip: {
        ...
        formatter: (datum: any) => {
          const typeInfo = availableDataTypes.find(t => t.label === datum.type)
        ...
        title: (title: any, datum: any) => {
          ...
          if (typeof title === 'string') {
            timeStr = title
          } else if (datum && datum.length > 0 && datum[0].time) {
            timeStr = datum[0].time
          } else if (title && title.time) {
            timeStr = title.time
          } else {
